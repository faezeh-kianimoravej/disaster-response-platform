...
**Date:** 2025-10-30
**Sprint:** 06
**Related ADRs:** ADR_Event_Driven_Notification_Workflow

## Context

Maintaining consistency in a large frontend codebase is challenging when multiple developers contribute without clear standards. Without established patterns, developers create custom solutions for common problems (pages, forms, API integration), leading to:

- Inconsistent naming and file organization
- Duplicated API logic and validation
- Unclear separation of concerns (mixing display, API, and form models)
- Difficulty onboarding new developers
- Higher maintenance costs and refactoring burden

## Decision

Adopt a consistent frontend architecture with clear separation of concerns and standard patterns for pages, components, hooks, API integration, forms, and types.
...
- Put validation logic at hook level for form hooks
...
- In addition to toasts, the UI area where the error occurred must show an inline, contextual message and an optional retry action
...
- When a data fetch fails for a page or a control, follow this explicit small contract so implementations remain consistent across the app:
  ...
  4. Inline retry controls MUST be keyboard-accessible native `<button>`s, include an `aria-label`, and offer a visible focus indicator
...
- Single responsibility: separate display, API, and form models
- Centralized auth & routing: use `AuthGuard` for page access control
- Small, reusable UI: prefer tiny components (LoadingPanel, SearchSelect)
- Hooks for encapsulating data & logic
- Typed boundaries: clearly name Display/API/Form types with adapters
- Validation: enforce at the hook level
- Visible UX: keep page chrome visible while loading

**Benefits:**

- **Consistency**: Standard patterns reduce cognitive load and enable faster code review
- **Maintainability**: Separation of concerns (API/Display/Form models, UI/logic) makes changes easier
- **Scalability**: Clear patterns enable teams to work independently on features
- **Testing**: Isolated hooks and components are easier to unit test
- **Onboarding**: New developers can reference this ADR to understand conventions
- **Type Safety**: Clear type boundaries prevent integration errors
- **Error Handling**: Consistent patterns for retry and error display

## Consequences

### Positive

- Consistent codebase structure across all pages and components
- Faster code reviews with clear checklists
- Easier onboarding for new frontend developers
- Reduced code duplication through reusable components and hooks
- Clear separation between API, display, and form models
- Better error handling with consistent inline error UI and toast patterns
- Type-safe boundaries reduce integration errors

### Negative / Trade-offs

- **Learning curve**: Developers must follow patterns (mitigated by ADR examples)
- **More files**: Separate API, adapter, and type files add to project structure
- **Validation duplication**: Both frontend and backend validation needed (necessary for UX)

## Alternatives Considered

### Alternative 1: No Type Separation (Single Model)

- Use same type for API, display, and forms
- **Pros**: Fewer type definitions
- **Cons**: Tight coupling to backend, API changes break UI, form fields leak into display
- **Why not chosen**: Does not provide flexibility or maintainability

### Alternative 2: Class-Based Components

- Use React class components instead of functional components
- **Pros**: Familiar to developers with older React experience
- **Cons**: More verbose, harder to test, no hooks support
- **Why not chosen**: Modern React ecosystem favors functional components and hooks

### Alternative 3: Redux for All State

- Use Redux for both server state and client state
- **Pros**: Single state management solution
- **Cons**: Boilerplate overhead, overkill for server state when React Query available
- **Why not chosen**: React Query better suited for server state; Context API sufficient for client state

## Validation / Testing

Implementation is validated through:

- **ESLint enforcement**: `no-console`, `react-hooks/rules-of-hooks`
- **TypeScript strict mode**: Catches type mismatches
- **Unit tests**: Components, hooks (with mocked fetch), adapters
- **Integration tests**: Key user flows (create, edit, delete)
- **Code review**: Use PR checklist in this ADR during reviews
- **Lint & typecheck**: Pass locally and in CI/CD
...
- ADR_Event_Driven_Notification_Workflow
- Example: `client-app/src/` structure
